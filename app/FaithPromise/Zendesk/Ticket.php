...
abstract class Ticket {
    ...
    private function sendViaEmail() {
        ...
        if ($recipient) {
            Mail::send(['text' => 'emails.ticket'], ['comment' => $this->buildDescription()], function($m) use ($recipient, $requester, $subject) {
                $m->subject($subject);
            ...
            $comment = 'Thank you for your request! If there are any questions or concerns with the request, you will hear from us within a few days. Otherwise, this will be entered into our project management system and a project lead will get to work and contact you for more information and keep you updated on your project!';
            $comment .= PHP_EOL . PHP_EOL . 'Also, feel free to reply and attach any files for this project.';

            Mail::send(['text' => 'emails.ticket'], ['comment' => $comment], function($m) use ($recipient, $requester, $subject) {
                $m->subject($subject);
                $m->to($requester->email, $requester->name);
                $m->from($recipient->email, $recipient->name);
                $m->replyTo($recipient->email, $recipient->name);
            });
        }
